...
# ============ НАСТРОЙКИ ============
TOKEN = "8356413290:AAGvwTj0fK8_QxwwrPHhB7Kdw6UlblbmECE"
DB_PATH = "fridge.db"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        with open("priv.jpg", "rb") as photo:
            caption = (
                "Что может делать этот бот?\n\n"
                "👨‍🍳 Напиши или сфоткай свои продукты и я отправлю вкусные рецепты под точным подсчётом КБЖУ:\n\n"
                "- Похудение\n"
                "- Правильное питание\n"
                "- До 15 мин\n"
                "- Обычные блюда\n"
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("Перейти на PRO за 1 ₽", callback_data="premium")],
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "cookbro":
        ...
        rows = cur.execute("SELECT DISTINCT name FROM products WHERE user_id = ? ORDER BY name", (query.from_user.id,)).fetchall()
